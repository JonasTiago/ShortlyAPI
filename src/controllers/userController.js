...
export async function myShortlys(req, res) {
  ...
  try {
    const myShorts = await connection.query(
      ...
      json_agg(json_build_object('id', (COALESCE (l.id, 0)), 'shortUrl',(COALESCE (l.shortly, '')) , 'url',(COALESCE (l.url, '')), 'visitCount', (COALESCE (l.visualizations, 0)))) AS "shortenedUrls"
      FROM users u LEFT JOIN urls l ON u.id = l."userId" WHERE u.id = $1 GROUP BY u.id;
    `,
      [userId]
    );
